...
for (Int_t i=0;i<entradas;i++){
    ...
    if (angle==0&Fidu<50){h1 -> Fill(ResZ);}
    else if (angle==0&Fidu<100){h2 -> Fill(ResZ);}
    else if (angle==1&Fidu<50){h1 -> Fill(ResA);}
    else if (angle==1&Fidu<100){h2 -> Fill(ResA);}
}

TString filtro1 = "rec.coreFiduScale<50";
TString filtro2 = "rec.coreFiduScale<100";
auto legend = new TLegend(0.4,0.76,0.5,0.87);
...
if (angle==0){
    h1 -> SetTitle("Resolucion cenital para reconstruir nucleos de cascadas iniciadas por fotones");
    h1 -> GetXaxis()->SetTitle("Angulo");}
if (angle==1){
    h1 -> SetTitle("Resolucion azimutal para reconstruir nucleos de cascadas iniciadas por fotones");
    h1 -> GetXaxis()->SetTitle("Angulo");};
